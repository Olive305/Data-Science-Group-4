...
def run_stratified_did(df, dependent_var='Mitglieder_diff_next', treatment_var='ZB_diff', time_var='Date'):
    ...
    features = ['treatment', 'post', 'treatment:post'] + controls
    model_bundle = {
        'model': model,
        'features': features,
        'add_constant': True,  # smf.ols automatically adds constant
        'scaler': None
    }

    # Save model bundle to file
    joblib.dump(model_bundle, '../models/did_stratified_model.pkl')
...
def did():
    """
    Load, clean and run DiD model on merged data.
    """
